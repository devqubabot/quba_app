...
- [ ] Domain entities use named types for IDs, dates, instants, progress values, and configuration versions without framework/vendor imports.
- [ ] A new linked progress event updates its matching run and occurrence; crossing the occurrence target creates exactly one 10 XP ledger entry and records one streak completion.
- [ ] Re-delivering the same `event_id` returns an acknowledgeable duplicate outcome without state mutations or a second reward.
- [ ] Several linked runs may contribute to one occurrence, while completing a partial run does not complete or reward the occurrence early.
- [ ] A standalone run can complete and enter processed history without an occurrence, XP ledger entry, or streak change.
- [ ] Mismatched links, non-positive progress, unavailable entities, and archived/disabled habits are rejected with typed, non-acknowledgeable outcomes.
- [ ] Habit configuration accepts only monotonically newer versions, distinguishes an identical retry from a same-version conflict, and prevents tombstone removal.
- [ ] The application use case defines an atomic unit-of-work port and translates transaction failure into a retryable typed outcome.
- [ ] Deterministic tests cover normal, boundary, duplicate, linked, standalone, invalid, stale-version, tombstone, and transaction-failure behavior.
- [ ] `npm run check` passes with no new production dependency.
...
The reviewed bootstrap baseline is established and this task brief is active. Domain and application implementation has not started.
...
Implement the domain types and configuration-version rules, then add reconciliation behavior and deterministic tests.
